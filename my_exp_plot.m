...
acc_features = 1:5;
for use_acc_num = 1
    use_acc_comb = nchoosek(acc_features, use_acc_num);
    for i = 1:size(use_acc_comb, 1)
        ...
        acc_num = 1;
        peak_win_num = 12;
        used_acc_features = use_acc_comb(i,:);
        ...
        exp_name = sprintf('on13acc_%s__%s', sprintf('%d', used_acc_features), date);
        ...
        for i = 13
            ...
            %train
            model = ...
                    my_svm_train(training_file, c, gamma, train_idxes, lastpredict_num, past_acc_end, acc_num, used_acc_features);

            %predict
            [mse_predict, corr_predict, aae_predict, tgt_label, out_label_predict] = ...
                    my_svm_predict(model, predict_file, output_file, i, use_lastpredict, lastpredict_num, past_acc_end, acc_num, used_acc_features, peak_win_num);
